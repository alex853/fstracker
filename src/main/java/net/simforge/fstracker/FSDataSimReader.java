...
public class FSDataSimReader implements Runnable, FSDataProvider {
    private final Queue<FSDataRecord> queue = new ConcurrentLinkedQueue<>();
...
//                            System.out.println(LocalDateTime.now() + " Could not connect to SIM");
                    queue.offer(new FSDataRecord(FSDataRecord.Status.NO_CX));
...
//                        System.out.println(LocalDateTime.now() + " Year = 0, disconnection is assumed");
                queue.offer(new FSDataRecord(FSDataRecord.Status.BAD_CX));
                fsuipc_wrapper.Close();
                fsuipc = null;
                connected = false;
                continue;
...
//                        System.out.println(LocalDateTime.now() + " READING IS TOO SLOW, disconnection is assumed");
                queue.offer(new FSDataRecord(FSDataRecord.Status.READ_TIMEOUT, fsDataRecord));
                fsuipc_wrapper.Close();
                fsuipc = null;
                connected = false;
                continue;
            ...
            queue.offer(new FSDataRecord(FSDataRecord.Status.OK, fsDataRecord));
        }
